 ...
 * */
//渲染文件夹
var aFile = $S('.m-file', filecon); //获取文件夹
function renderFile(id) {
    var sHtml = '';
    datas.forEach(function (item, index) {
        if (item.pid === id) sHtml += fileHtml(item);
    });
    filecon.innerHTML = sHtml;
    //if($S('.dragselect')[0]){   //小概率事件，非常小的几率出现拖拽选择框不消失的情况，渲染时删除
    //    document.body.removeChild($S('.dragselect')[0]);
    //}
}
//文件夹添加点击事件,事件委托
